...
$query = "UPDATE examination_scheme set Valid=0, `Updated on`=CURRENT_TIMESTAMP
            where SubjectDetailsID=? and SemesterID=? and Valid=1";
$stmt = $conn->prepare($query);
$stmt->bind_param("ii", $subjectdetailsID, $semesterNumber);
...
$stmt->close();
echo 'Examination Scheme deleted successfully';
return true;
?>
